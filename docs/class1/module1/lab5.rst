...
このラボでは、下記の手順に従ってクラスタを作成します。このような複雑な操作は、 **Imperative（命令型）モデル** を使用すると、その有効性が低下します。クラスタリングの設定は、ほとんどの顧客が **Declarative（宣言型）モデル** に移行するためのきっかけの1つです。なぜなら、自動化を利用するユーザーからデバイス/ベンダーレベルの詳細情報を開示せず、抽象化する必要があるためです。
