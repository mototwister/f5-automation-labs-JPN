...
#. 出力結果をみて、どのようにワークフローが実行されたかを確認します。ここでのテストは、これまでのPostmanを使用したときのテストと同じであることに注目してください。
...
``bigip_version`` と ``bigip_build`` 変数が保存されてことを確認してください。このファイルはJSONでフォーマットされているので、他のツールで簡単に使用することができます。
